...
def measure_shs_squared(psi, sigma, lattice, model_params, site=0):
    """
    ...
    1 - x - 3 - y - 5 - x - 7 - y - 9 - x - 11 - y - 13 - x - 15 - y - 17 - x - 19
    |       |       |       |       |       |        |        |        |        |
    z       z       z       z       z       z        z        z        z        z
    |       |       |       |       |       |        |        |        |        |
    0 - y - 2 - x - 4 - y - 6 - x - 8 - y - 10 - x - 12 - y - 14 - x - 16 - y - 18
    """
    ...
    print("First application: compute |psi> = sigma |phi> ...")

    print("Applying on-site operators:")
    op, s = op_Ax[0]; print(op, s)
    print("Applying on-site Pauli operator at site ", s)
    psi.apply_local_op(s, sigma, unitary=False, renormalize=False)
    psi_tmp = psi.copy()
...
def main(**kwargs):
    ...
    energy, energy_sq = measure_shs_squared(psi, sigma, M.lat, model_params, site=j-2)
    print(energy)
...
if __name__ == "__main__":
    ...
    op_type = 'Sigmax'
    gs_file = "./ground_states/GS_L10defaultchi90_K-1.00Fx-0.70Fy-0.70Fz-0.70.h5"
