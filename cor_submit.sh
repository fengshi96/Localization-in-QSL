...
#SBATCH --job-name cor_h$h                  # descriptive name shown in queue and used for output files
...
time python ../correlator_dynamics.py $(pwd)/ground_states/GS_L${Lx}${Ly}Cstylechi${chi}_K-1.00Fx${h}Fy${h}Fz${h}.h5 $op &> corDyn.out


EOF
)

	echo "$rawjob" &> jobCorDyn.bat
 	sbatch jobCorDyn.bat
	echo "Correlator Dynamics for h=$h is submitted"
	cd ..
done
