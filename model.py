...
class Kitaev_Extended(CouplingMPOModel):
    ...
    def init_terms(self, model_params):
        ...
        edge_polarization = model_params.get('edge_polarization', False)
        ...
        ops = [('Sigmay', [0, 0], 0), ('Sigmaz', [0, 0], 1), ('Sigmax', [1, 0], 0), ('Sigmay', [1, -1], 1), ('Sigmaz', [1, -1], 0), ('Sigmax', [0, -1], 1)]
        # ops = [('Sigmaz', [0, 0], 0), ('Sigmax', [0, 0], 1), ('Sigmay', [1, 0], 0), ('Sigmaz', [1, -1], 1), ('Sigmax', [1, -1], 0), ('Sigmay', [0, -1], 1)]
        self.add_multi_coupling(-W, ops)

        if edge_polarization:
            print("Achtung! I'm polarizing the left edge indexed by " + " ".join(str(i) for i in range(0, 2 * model_params['Ly'])))
            for i in range(0, 2 * model_params['Ly']):
                self.add_onsite_term(1000, i, 'Sigmax')
                self.add_onsite_term(1000, i, 'Sigmay')
                self.add_onsite_term(1000, i, 'Sigmaz')
