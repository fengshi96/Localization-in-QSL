...
# constructing and solving majorana Hamiltonian for the gs sector
    if coloring_solution is not None:
        maj_ham = ham.majorana_hamiltonian(modified_lattice, coloring_solution, ujk) * 2
    else:
        maj_ham = ham.majorana_hamiltonian(modified_lattice, None, ujk) * 2
...
def main(total, cmdargs):
    ...
    target_flux = []
    prob = 0.8
    for p in modified_lattice.plaquettes:
        if p.n_sides == 3:
            if np.random.rand() < prob:
                target_flux.append(1)
            else:
                target_flux.append(-1)
        else:
            target_flux.append(-1)
    target_flux = np.array(target_flux, dtype=np.int8)
    print(target_flux)
    ...
    plot_edges(modified_lattice, ax= ax1, labels=coloring_solution, directions=ujk, lw=0.5, alpha=1)
    # plot_edges(modified_lattice, ax= ax1,labels=coloring_solution, directions=ujk)
    plot_plaquettes(modified_lattice, ax=ax1, labels = fluxes_to_labels(fluxes), color_scheme=np.array(['grey','w','wheat', 'thistle']))
    # plot_vertex_indices(modified_lattice, ax= ax1)

    plt.savefig("test_tri_ham_figure.pdf", dpi=900,bbox_inches='tight')
    ...
    plt.savefig("sierpinski_dos.pdf", dpi=300,bbox_inches='tight')
