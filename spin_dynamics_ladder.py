...
def measure_evolved_spin(measurements, evolved_time, env, M, model_params, site='all'):
    ...
    measurements[t_key]['energy'] = measure_spin_densities(env.ket, M.lat, model_params, site)
    ...
    if t_method == 'ExpMPO':
        ...
        for i in range(tsteps_cont):
            ...
            if i%100 == 0 or i == tsteps_cont - 1:  # i == tsteps_cont - 1: #i%2 == 0:
                ...
                if save_state:
                    with h5py.File(f'./spin_time_states/{tsteps_init+previous_evolved_steps+i+1}'+f'time{round(eng.evolved_time, 2)}'+t_name, 'w') as f:
